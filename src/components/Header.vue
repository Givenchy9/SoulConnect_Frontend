<template>
    <nav class="w-full h-28 bg-orange-500 opacity-80 flex items-center justify-between px-8 relative">
      ...
      <div class="relative lg:block hidden">
        <div @click="toggleDropdown" class="flex items-center text-2xl space-x-4 cursor-pointer">
          ...
          <span class="text-white text-lg">Mohamad</span>
        </div>
    ...
    <div v-if="mobileMenuOpen" class="lg:hidden absolute top-28 left-0 w-full bg-orange-500 opacity-80 px-8 py-4">
      ...
      <RouterLink to="/notificatie" class="block text-white text-lg py-2">
        <i class="fa-solid fa-bell text-white h-8 w-8"></i> Notificaties <!-- Zorg dat notificatie-icoon hier ook groter is -->
      </RouterLink>
  
      <div @click="toggleDropdown" class="block text-white text-lg py-2 cursor-pointer">
        <i class="fas fa-user-circle text-white h-8 w-8"></i> Profiel <!-- Zorg ervoor dat het profiel-icoon hier ook groter is -->
      </div>
